...
function transformToLis(obj){
  // Solution code here...
   let arr = Object.entries(obj);
    return arr.map(item => (`<li>${item[0]}: ${item[1]}</li>`));
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat(2).reduce((acc, val) =>  (acc + val), 0)
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names = [];
  data.forEach(nameByGender => {
    if(nameByGender.gender === 'male' || nameByGender.gender === 'female'){
      names.push(nameByGender.name);
    }
  })
