...
const toLastNames = people => {
  // Solution code here...
    return people.map(string => `${string.firstName} ${string.lastName}`);
};
...
const addPurchases = (arr) => {
  // Solution code here...
  let purchased = arr.map(string => string.purchasePrice)
  let total = purchased.reduce((acc, val) => acc + val, 0);
  return total;
};
...
const returnNames = (arr) => {
  let starWars = arr.reduce((acc, val) => { acc.push(val.name)
    return acc;
  }, []);
  return starWars;
