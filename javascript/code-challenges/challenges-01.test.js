...
const addOne = (arr) => {
  let array = [];
  arr.forEach(num => array.push (num + 1));
  return array;
};
...
const allUpperCase = (arr) => {
  let array = [];
  arr.forEach(string => array.push (string.toUpperCase()));
  return array;
};
...
const speaker = (words, callback) => {
  let array = [];
  words.forEach(n => array.push(callback(n)));
  return array;
};
...
const createList = (availableItems) => {
  // Solution code here...
let finalList = [];
availableItems.forEach(item => array.push())
  return finalList;
};
